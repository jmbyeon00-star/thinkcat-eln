...
from app.models.subscription import OrganizationDB
...
async def _prepare_idea(raw_text: str, section: str, n: int, db: AsyncSession, pdf_name: str | None = None) -> dict:
    # base_id 먼저 계산 (캐시 키)
    if pdf_name:
    ...
    # DB 캐시 확인
    db_r = await db.execute(select(InvalPrepareCacheDB).where(InvalPrepareCacheDB.base_id == base_id))
...
async def prepare_from_text(payload: dict, db: AsyncSession = Depends(get_async_session)):
...
# ─────────────────────────────────
# 아이디어 기반 - 보고서 스트리밍 (SSE)
# ─────────────────────────────────

@router.post("/analysis/prior-art-report/stream")
async def generate_prior_art_report_stream(payload: dict, request: Request, db: AsyncSession = Depends(get_async_session)):
    """prior-art-report와 동일하나 분석 진행 상황을 SSE로 스트리밍.
    - text 이벤트: 현재 단계 상태 메시지
    - anchor_done 이벤트: 구성요소 쌍별 분석 완료 시 (프론트 점진적 UI용)
    - done 이벤트: 최종 결과 포함"""
        ...
        try:
            ...
            if user_id:
                ...
                if not sub_status["ok"]:
                    ...
                    if reason == "expired_trial":
                        await log_usage(db, None, user_id, base_id, "prior_art_search_expired", reason="expired_trial")
                        yield sse({"error": "무료 체험이 종료되었습니다", "code": "expired_trial"}); return
                    else:
                        await log_usage(db, None, user_id, base_id, "prior_art_search_expired", reason="expired_subscription")
                        yield sse({"error": "구독이 만료되었습니다", "code": "expired_subscription"}); return
                org_id = sub_status["org_id"]
                subscription_started_at = sub_status.get("subscription_started_at")

                # 같은 조직 + 구독 기간 내 재조회 → 크레딧 차감 없이 반환
                if await is_cached_for_user(db, org_id, base_id, subscription_started_at):
                    if report_cache_key:
                        db_rc = await db.execute(
                            select(InvalPriorArtReportDB)
                            .where(
                                InvalPriorArtReportDB.base_id == report_cache_key,
                                InvalPriorArtReportDB.model   == model,
                        ...
                        cached_c = db_rc.scalars().first()
                        if cached_c:
                            cached_result_c = json.loads(cached_c.result_json)
                            await log_usage(db, org_id, user_id, base_id, "prior_art_search_cached")
                            yield sse({"done": True, "result": cached_result_c}); return
            ...
            if report_cache_key:
                ...
                if cached:
                    cached_result = json.loads(cached.result_json)
                    history_id = await create_idea_history(db, user_id, base_id, report_cache_key, model, idea_title, cached_result, prior_app_numbers, result_json=cached.result_json, org_id=org_id)
                    if user_id:
                        ok = await decrement_credit(db, org_id, user_id, base_id, history_id or 0)
                        if not ok:
                            yield sse({"error": "크레딧 차감에 실패했습니다", "code": "no_credits"}); return
                    yield sse({"done": True, "result": cached_result})
                    return

            # ① 앵커 추출
            yield sse({"text": "청구항 구성요소 분석 중..."})
            async with httpx.AsyncClient(timeout=180) as client:
                anchor_res = await client.post(
                    f"{settings.GPU_BACKEND_URL}/gpu/invalidation/parse/anchors",
                    json={"raw_text": raw_text, "full_text": full_text, "model": model},
            ...
            if not anchor_res.is_success:
                ...
                yield sse({"error": f"앵커 추출 실패: {anchor_res.text[:200]}"}); return
            anchors = anchor_res.json()
            if not anchors:
                if user_id:
                    await log_usage(db, org_id, user_id, base_id, "prior_art_search_failed", reason="invalid_input")
                yield sse({"error": "앵커를 추출할 수 없습니다"}); return
            yield sse({"text": " / ".join(a.get("name", "") for a in anchors)})

            # ② 선행발명 파싱 (병렬, as_completed)
            def _make_parse_task(app_num: str):
                async def _task():
                    db_r2 = await db.execute(
                        select(InvalElementDB).where(
                            InvalElementDB.application_number == app_num,
                    ...
                    existing = db_r2.scalars().all()
                    if existing:
                        return app_num, [{"id": e.element_key, "name": e.name or "", "embedding_text": e.embedding_text or e.function or "", "criticality": e.criticality or 3, "source_claim": e.source_claim, "raw_text": e.raw_text or ""} for e in existing]
                    prior_info = await patent_parser.get_patent_info(app_num, db)
                    ...
                    async with httpx.AsyncClient(timeout=180) as client:
                        res = await client.post(
                            f"{settings.GPU_BACKEND_URL}/gpu/invalidation/parse/prior-search",
                            json={"patent_info": {k: str(v) if v is not None else None for k, v in prior_info.items()}, "claims_text": claims_text, "base_anchors": [{"id": a.get("id", f"E{i+1}"), "name": a.get("name", ""), "embedding_text": a.get("embedding_text", "")} for i, a in enumerate(anchors)], "model": model},
                        )
                    if not res.is_success: return app_num, []
                    elements = res.json()
                    await _save_elements(db, app_num, base_id, elements, model)
                    return app_num, [{"id": e.get("id"), "name": e.get("name", ""), "embedding_text": e.get("embedding_text") or e.get("function", ""), "criticality": e.get("criticality", 3), "source_claim": e.get("source_claim"), "raw_text": e.get("raw_text") or ""} for e in elements]
                return asyncio.create_task(_task())

            parse_tasks = [_make_parse_task(n) for n in prior_app_numbers]
            prior_elements_map: dict[str, list] = {}
            parsed_count = 0
            for coro in asyncio.as_completed(parse_tasks):
                try:
                    ...
                    if elems:
                        prior_elements_map[app_num] = elems
                        parsed_count += 1
                        yield sse({"text": f"선행발명 파싱 중... ({parsed_count}/{len(prior_app_numbers)})"})
                except Exception as e:
                    print(f"⚠️ 파싱 오류: {e}")
            ...
            if not prior_elements_map:
                if user_id:
                    await log_usage(db, org_id, user_id, base_id, "prior_art_search_failed", reason="invalid_input")
                yield sse({"error": "선행발명 구성요소를 추출할 수 없습니다"}); return

            # ③ 배치 임베딩 + 유사도 계산
            yield sse({"text": "유사도 계산 중..."})
            THRESHOLD = 0.65
            anchor_texts = [a.get("embedding_text") or a.get("name", "") for a in anchors]
            prior_ordered = list(prior_elements_map.items())
            ...
            for app_num, elems in prior_ordered:
                texts = [e.get("embedding_text") or e.get("name", "") for e in elems]
                raw_texts = [e.get("raw_text") or "" for e in elems]
                prior_flat_texts.extend(texts); prior_flat_raw_texts.extend(raw_texts)
                prior_boundaries.append((app_num, offset, offset + len(texts))); offset += len(texts)
            ...
            embed_res.raise_for_status()
            raw_vecs = np.array(embed_res.json()["embeddings"], dtype=np.float32)
            norms = np.linalg.norm(raw_vecs, axis=1, keepdims=True); norms = np.where(norms == 0, 1, norms)
            all_vecs = raw_vecs / norms; anchor_vecs = all_vecs[:len(anchor_texts)]

            def _level(s): return "높음" if s >= 0.80 else ("보통" if s >= THRESHOLD else "낮음")
            ...
            per_anchor_data = []
            for i, anchor in enumerate(anchors):
                a_vec = anchor_vecs[i:i+1]; scores = []
                for app_num, start, end in prior_boundaries:
                    elems = prior_elements_map[app_num]; p_vecs = all_vecs[start:end]
                    sims = (a_vec @ p_vecs.T)[0]; best_idx = int(np.argmax(sims))
                    flat_idx = start - len(anchor_texts) + best_idx; max_sim = float(sims[best_idx])
                    scores.append({"patent_id": app_num, "title": prior_infos.get(app_num, {}).get("title", app_num), "similarity": round(max_sim, 4), "similarity_level": _level(max_sim), "best_element_name": elems[best_idx].get("name", ""), "best_element_text": prior_flat_texts[flat_idx], "best_element_raw_text": prior_flat_raw_texts[flat_idx], "best_element_claim": elems[best_idx].get("source_claim")})
                scores.sort(key=lambda x: -x["similarity"])
                anchor_max = scores[0]["similarity"] if scores else 0.0
                per_anchor_data.append({"anchor_id": anchor.get("id", f"E{i+1}"), "anchor_name": anchor.get("name", ""), "anchor_text": anchor.get("embedding_text", ""), "criticality": anchor.get("criticality", 3), "similarity_level": _level(anchor_max), "top_priors": scores[:3]})

            related_patents = list(dict.fromkeys(p["patent_id"] for a in per_anchor_data for p in a["top_priors"] if p["similarity"] >= THRESHOLD))
            uncovered_anchors = [a["anchor_name"] for a in per_anchor_data if not any(p["similarity"] >= THRESHOLD for p in a["top_priors"])]

            # ④ 쌍별 분석 — as_completed + anchor_done 이벤트
            def _pair_hash(text): return hashlib.md5(text.encode()).hexdigest()

            anchor_pair_needs: dict[str, list[tuple[str, str]]] = {
                a_data["anchor_id"]: [
            ...
            def _build_anchor_event(a_data: dict) -> dict:
                prior_comps = []
                for prior in a_data["top_priors"]:
                    ah = _pair_hash(a_data["anchor_text"]); eh = _pair_hash(prior["best_element_text"])
                    pr = pair_cache.get((ah, eh), {})
                    prior_comps.append({"patent_id": prior["patent_id"], "patent_title": prior["title"], "matched_element": prior["best_element_name"], "operation": prior["best_element_text"], "similarity": pr.get("similarity", ""), "avoidance_direction": pr.get("avoidance_direction", "")})
                return {"event": "anchor_done", "data": {"anchor_id": a_data["anchor_id"], "anchor_name": a_data["anchor_name"], "similarity_level": a_data["similarity_level"], "prior_comparisons": prior_comps}}

            pairs_to_call, pair_cache = [], {}
            for a_data in per_anchor_data:
                for prior in a_data["top_priors"]:
                    a_hash = _pair_hash(a_data["anchor_text"]); e_hash = _pair_hash(prior["best_element_text"])
                    db_r3 = await db.execute(
                        select(InvalPairAnalysisDB).where(
                            InvalPairAnalysisDB.anchor_text_hash  == a_hash,
                            InvalPairAnalysisDB.element_text_hash == e_hash,
                    ...
                    cached_pair = db_r3.scalars().first()
                    if cached_pair:
                        pair_cache[(a_hash, e_hash)] = {"similarity": cached_pair.similarity, "avoidance_direction": cached_pair.avoidance_direction}
                    else:
                        pairs_to_call.append((a_data, prior, a_hash, e_hash))

            yield sse({"text": f"구성요소 쌍별 분석 중... (0/{len(per_anchor_data)})"})

            # 캐시로 완성된 앵커 즉시 anchor_done 발송
            for a_data in per_anchor_data:
                anchor_id = a_data["anchor_id"]
                if anchor_id not in anchor_done_set and all((ah, eh) in pair_cache for ah, eh in anchor_pair_needs.get(anchor_id, [])):
                    anchor_done_set.add(anchor_id)
                    yield sse(_build_anchor_event(a_data))

            async def _call_pair(a_data, prior, a_hash, e_hash):
                async with httpx.AsyncClient(timeout=120) as client:
                    res = await client.post(f"{settings.GPU_BACKEND_URL}/gpu/invalidation/pair-analysis", json={"anchor_name": a_data["anchor_name"], "anchor_text": a_data["anchor_text"], "element_name": prior["best_element_name"], "element_text": prior["best_element_text"], "element_raw_text": prior["best_element_raw_text"], "similarity_score": prior["similarity"], "similarity_level": prior["similarity_level"], "model": model})
                if not res.is_success: return a_hash, e_hash, {"similarity": "", "avoidance_direction": ""}
                return a_hash, e_hash, res.json()
            ...
            for coro in asyncio.as_completed(pair_tasks):
                try:
                    ...
                    try:
                        db.add(InvalPairAnalysisDB(id=str(uuid.uuid4()), anchor_text_hash=a_hash, element_text_hash=e_hash, model=model, similarity=pair_result.get("similarity", ""), avoidance_direction=pair_result.get("avoidance_direction", "")))
                    except Exception: pass
                    # 새로 완성된 앵커 anchor_done 발송
                    for a_data in per_anchor_data:
                        anchor_id = a_data["anchor_id"]
                        if anchor_id not in anchor_done_set and all((ah2, eh2) in pair_cache for ah2, eh2 in anchor_pair_needs.get(anchor_id, [])):
                            anchor_done_set.add(anchor_id)
                            yield sse(_build_anchor_event(a_data))
                            yield sse({"text": f"구성요소 분석 완료 ({len(anchor_done_set)}/{len(per_anchor_data)})"})
                except Exception as e:
            ...
            for a_data in per_anchor_data:
                ...
                for prior in a_data["top_priors"]:
                    a_hash = _pair_hash(a_data["anchor_text"]); e_hash = _pair_hash(prior["best_element_text"])
                    pair_result = pair_cache.get((a_hash, e_hash), {})
                    prior_comparisons.append({"patent_id": prior["patent_id"], "patent_title": prior["title"], "matched_element": prior["best_element_name"], "operation": prior["best_element_text"], "similarity": pair_result.get("similarity", ""), "avoidance_direction": pair_result.get("avoidance_direction", "")})
                anchor_analyses.append({"anchor_id": a_data["anchor_id"], "anchor_name": a_data["anchor_name"], "similarity_level": a_data["similarity_level"], "prior_comparisons": prior_comparisons})

            # ⑥ 종합 검토의견
            yield sse({"text": "권리화 종합 전략 작성 중..."})
            related_patents_list = [{"patent_id": pid, "title": prior_infos.get(pid, {}).get("title", pid)} for pid in related_patents]
            async with httpx.AsyncClient(timeout=180) as client:
                overall_res = await client.post(f"{settings.GPU_BACKEND_URL}/gpu/invalidation/overall-synthesis", json={"idea_full_text": full_text, "anchors": [{"id": a.get("id", f"E{i+1}"), "name": a.get("name", ""), "criticality": a.get("criticality", 3)} for i, a in enumerate(anchors)], "per_anchor_data": anchor_analyses, "related_patents": related_patents_list, "uncovered_anchors": uncovered_anchors, "model": model})
            if not overall_res.is_success:
                if user_id:
                    await log_usage(db, org_id, user_id, base_id, "prior_art_search_failed", reason="gpu_error")
                yield sse({"error": f"종합 검토 생성 실패: {overall_res.text[:200]}"}); return

            result = {"idea_uuid": idea_uuid, "base_id": base_id, "full_text": full_text, "anchors": anchors, "prior_infos": prior_infos, "per_anchor_data": per_anchor_data, "related_patents": related_patents_list, "uncovered_anchors": uncovered_anchors, "report": {"anchor_analyses": anchor_analyses, "overall": overall_res.json()}}
            result_json_str = json.dumps(result, ensure_ascii=False, default=str)
            ...
            if report_cache_key:
                try:
                    ...
                    await db.rollback(); print(f"⚠️ 보고서 저장 실패: {e}")

            final_result = json.loads(result_json_str)
            history_id = await create_idea_history(db, user_id, base_id, report_cache_key, model, idea_title, final_result, prior_app_numbers, result_json=result_json_str, org_id=org_id)
            if user_id:
                await decrement_credit(db, org_id, user_id, base_id, history_id or 0)
            yield sse({"done": True, "result": final_result})

        except asyncio.TimeoutError as e:
            if user_id:
                await log_usage(db, org_id, user_id, base_id, "prior_art_search_failed", reason="gpu_timeout")
            yield sse({"error": "분析 시간이 초과되었습니다"})
        except Exception as e:
...
async def list_idea_history(
    ...
    """조직 전체 선행기술조사 내역 목록 (최신순). my_only=true면 본인 것만."""
    ...
    filter_user_id = user_id if my_only else None
    return await get_idea_history_list(db, sub_status["org_id"], user_id=filter_user_id, skip=skip, limit=limit, subscription_started_at=subscription_started_at)
...
async def get_idea_history(
    ...
    result = await get_idea_history_detail(db, history_id, user_id, org_id=org_id)
    if result is None:
...
async def remove_idea_history(
    ...
    """선행기술조사 히스토리 삭제"""
    deleted = await delete_idea_history(db, history_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="히스토리를 찾을 수 없습니다")
    return {"ok": True}
...
async def refresh_idea_history(
    ...
    db_r = await db.execute(
        select(InvalIdeaHistoryDB).where(
            InvalIdeaHistoryDB.id                   == history_id,
            InvalIdeaHistoryDB.requested_by_user_id == user_id,
        )
    )
    history = db_r.scalars().first()
    if not history:
        raise HTTPException(status_code=404, detail="히스토리를 찾을 수 없습니다")
    ...
    db_r2 = await db.execute(
        select(InvalPrepareCacheDB).where(InvalPrepareCacheDB.base_id == history.base_id)
    )
    prepare_cache = db_r2.scalars().first()
    ...
    async def _load_elements_from_db(app_num: str) -> list:
        db_r3 = await db.execute(
            select(InvalElementDB).where(
                InvalElementDB.application_number == app_num,
    ...
    for app_num in existing_in_new:
        elems = await _load_elements_from_db(app_num)
        if elems:
            prior_elements_map[app_num] = elems
    ...
    async def _parse_new_patent(app_num: str) -> tuple[str, list]:
        ...
        if not res.is_success:
            return app_num, []
        elements = res.json()
        await _save_elements(db, app_num, history.base_id, elements, model)
        return app_num, [
            {"id": e.get("id"), "name": e.get("name", ""), "embedding_text": e.get("embedding_text") or e.get("function", ""), "criticality": e.get("criticality", 3), "source_claim": e.get("source_claim"), "raw_text": e.get("raw_text") or ""}
            for e in elements
        ]
    ...
    for a_data, prior, a_hash, e_hash in pair_keys:
        ...
        cached_pair = db_r4.scalars().first()
        if cached_pair:
            pair_cache[(a_hash, e_hash)] = {
                "similarity":          cached_pair.similarity,
                "avoidance_direction": cached_pair.avoidance_direction,
    ...
    await update_idea_history_result(db, history_id, user_id, new_result_json_str, new_prior_app_numbers)
    if user_id:
...
async def get_analysis_status(base_app_number: str, model: str = "claude", db: AsyncSession = Depends(get_async_session)):
    ...
    if existing:
        ...
        return {
            "cached":      True,
            "unavailable": False,
            "base":        await patent_parser.get_patent_info(base_app_number, db),
            "priors":      priors,
        }
...
async def _save_elements(db: AsyncSession, app_number: str, base_app_number: str, elements: list, model: str = "claude"):
    await db.execute(
        delete(InvalElementDB).where(
            InvalElementDB.application_number == app_number,
            InvalElementDB.base_app_number    == base_app_number,
            InvalElementDB.model              == model,
        )
...
async def _load_overrides(db: AsyncSession, session_id: Optional[str], base_app_number: str, model: str = "claude") -> tuple[dict, list]:
...
async def _generate_interpretation(base_elements, prior_elements_map: dict, greedy_result: dict, db: AsyncSession, model: str = "claude") -> str:
