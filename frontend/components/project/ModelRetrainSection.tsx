...
import { FileBarChart2, ChevronUp, ChevronDown, Sparkles, Upload, Database, CheckCircle2 } from "lucide-react";
import ModelProgressSSE from "@/components/ModelProgressSSE";
import FileUploadEditor from "@/components/common/FileUploadEditor"; // 해당 컴포넌트가 존재한다고 가정
...
export default function ModelRetrainSection({
    ...
    }, [model?.id, token, showRetrain, API_BASE]);
    ...
    const handleReTrain = async () => {
        ...
        try {
            ...
            formData.append("run_type", "retrain");

            const response = await fetch(
                `${API_BASE}/api/ai/retrain/classification/${model.data_scope}/${model.data_id}`,
                {
                    method: "POST",
                    headers: {
                        Authorization: `Bearer ${token}`,
        ...
        <div className="rounded-xl border border-zinc-200 bg-white shadow-sm overflow-hidden">
            ...
            >
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-lg font-bold text-zinc-800">추가 학습</span>
                </div>
                {showRetrain ? <ChevronUp className="h-5 w-5 text-zinc-400" /> : <ChevronDown className="h-5 w-5 text-zinc-400" />}
            </button>
                            ...
                            {isBusy && (storeStatus === "RUNNING" || storeStatus === "INFERRING") && model?.id ? (
                                ...
                                <div>
                                    <h3 className="text-lg font-bold text-zinc-900 mb-4">
                                        {storeStatus === "RUNNING" && "신규 데이터 학습 중..."}
                                        {storeStatus === "INFERRING" && "추론 중..."}
                                    </h3>
                                    <ModelProgressSSE
                                        targetId={model.id}
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <label className="flex flex-col">
                                                ...
                                                <input
                                                    ...
                                                    disabled={isBusy}
                                                    className="rounded-lg border border-zinc-200 px-3 py-2 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-100 disabled:bg-zinc-100"
                                                />
                                            </label>
                                            <label className="flex flex-col">
