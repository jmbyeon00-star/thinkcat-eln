...
export default function ProjectLayout({
  ...
  const getActiveTab = () => {
    ...
    if (pathname.includes("/data") || pathname.includes("/collection") || pathname.includes("/preview") || pathname.includes("/stats")) {
      return "data";
    }
    return "home";
  };
          ...
          <div className="flex flex-col md:flex-row items-start md:items-end gap-4 md:gap-6 m-3 md:mt-5 pb-6">
            ...
            <div className="flex-grow pb-2">
              ...
              <div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-zinc-600">
                ...
                {finalSourceType && <span>{finalSourceType.toUpperCase()}</span>}
              </div>
              <p className="mt-2 text-sm text-zinc-600">
                {finalProjectDesc || "프로젝트 설명 없음"}
              </p>

              <p className="flex mt-2 text-sm text-zinc-600">
                <Calendar className="h-4 w-4" />&nbsp;
                {finalCreatedDateTime ? setDatetimeToDate(finalCreatedDateTime) : '정보 없음'} &#183; {finalUpdatedDateTime ? setDatetimeToDate(finalUpdatedDateTime) : '정보 없음'}</p>
