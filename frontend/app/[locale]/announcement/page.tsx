        ...
        return (
            <div className="min-h-screen flex items-center justify-center">
                <div className="text-center">
                    <div className="animate-spin rounded-full h-16 w-16 border-b-2 border-[#4169E1] mx-auto mb-4"></div>
        ...
        return (
            <div className="min-h-screen flex items-center justify-center">
                <div className="text-center bg-white p-8 rounded-xl shadow-lg max-w-md">
                    ...
                    <button onClick={() => window.location.reload()} className="px-6 py-2 bg-[#4169E1] text-white rounded-lg hover:bg-[#3558C4] transition">다시 시도</button>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50">
            <main className="max-w-7xl mx-auto px-6 py-8">
                <h2 className="text-3xl font-bold text-gray-900 mb-8">국가 R&D 과제</h2>

                {/* Search Section */}
                <div className="bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] p-8 mb-8 border border-gray-100">
                    <form onSubmit={(e) => { e.preventDefault(); handleSearch(); }}>
                        <div className="space-y-6">
                            <div>
                                <label className="flex items-center text-sm font-semibold text-gray-700 mb-2">
                                    <span className="w-1 h-4 bg-[#5B7FFF] mr-2"></span>공고명
                                </label>
                                <div className="relative">
                                    <input
                                        ...
                                        className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl outline-none hover:border-[#5B7FFF] focus:border-[#5B7FFF] focus:shadow-[0_0_15px_rgba(91,127,255,0.3)] text-gray-900 bg-white"
                                    />
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div>
                                    <label className="flex items-center text-sm font-semibold text-gray-700 mb-2">
                                        <span className="w-1 h-4 bg-[#5B7FFF] mr-2"></span>공고기관명
                                    </label>
                                    <select
                                        value={tempSelectedAgency}
                                        onChange={(e) => setTempSelectedAgency(e.target.value)}
                                        className={`w-full px-4 py-3 border-2 border-gray-200 rounded-xl outline-none hover:border-[#5B7FFF] focus:border-[#5B7FFF] bg-white ${tempSelectedAgency === '' ? 'text-gray-400' : 'text-gray-900'}`}
                                    >
                                ...
                                <div>
                                    <label className="flex items-center text-sm font-semibold text-gray-700 mb-2">
                                        <span className="w-1 h-4 bg-[#5B7FFF] mr-2"></span>진행
                                    </label>
                                    <div className="flex items-center gap-8 px-4 py-3 h-[52px]">
                                        {['전체', '접수중', '접수예정'].map((status) => (
                                            <label key={status} className="flex items-center">
                                                <input type="radio" name="tempStatus" value={status} checked={tempSelectedStatus === status} onChange={(e) => setTempSelectedStatus(e.target.value)} className="w-4 h-4 text-[#5B7FFF]" />
                                                <span className="ml-2 text-gray-900 text-sm">{status}</span>
                                            </label>
                                        ))}
                                    </div>
                                </div>
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div>
                                    <label className="flex items-center text-sm font-semibold text-gray-700 mb-2">
                                        <span className="w-1 h-4 bg-[#5B7FFF] mr-2"></span>공고일
                                    </label>
                                    <div className="flex items-center gap-3">
                                        <input type="date" value={tempStartDate} onChange={(e) => setTempStartDate(e.target.value)} className="flex-1 px-4 py-3 border-2 border-gray-200 rounded-xl outline-none text-gray-400" />
                                        <span className="text-gray-700 text-sm">~</span>
                                        <input type="date" value={tempEndDate} onChange={(e) => setTempEndDate(e.target.value)} className="flex-1 px-4 py-3 border-2 border-gray-200 rounded-xl outline-none text-gray-400" />
                                    </div>
                                </div>
                                <div className="flex items-end">
                                    <button onClick={handleSearch} className="px-16 py-3 bg-gradient-to-r from-[#4169E1] to-[#5B7FFF] text-white font-semibold rounded-xl transform hover:scale-105 h-[52px]">검색</button>
                                </div>
                            </div>
                        </div>
                    </form>
                ...
                {/* Agency Shortcuts */}
                <div className="mb-8">
                    <h3 className="text-xl font-bold text-gray-800 mb-4">공고기관 바로가기</h3>
                    <div className="grid grid-cols-2 md:grid-cols-5 lg:grid-cols-10 gap-3">
                        {agencyStatsTop10.map((stat) => (
                            <div key={stat.name} className="flex flex-col items-center">
                                <button onClick={() => toggleAgency(stat.name)} className={`w-full p-3 rounded-lg border-2 transition-all ${selectedAgency.includes(stat.name) ? 'bg-gradient-to-br from-[#4A5CFF] to-[#3A47D5] text-white' : 'bg-[#F0F7FF] text-gray-800 border-none'}`}>
                                    <div className="text-[10px] font-medium line-clamp-2 h-8">{stat.name}</div>
                                </button>
                                <div className="text-[11px] text-gray-500 mt-1">({stat.count})</div>
                            </div>
                        ))}
                    </div>
                </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            <thead className="bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200">
                                <tr>
                                    <th className="px-2 py-3 text-center text-xs font-bold text-gray-700 uppercase w-3"></th>
                                    <th className="px-2 py-3 text-center text-xs font-bold text-gray-700 uppercase w-65">공고명</th>
                                    <th className="px-2 py-3 text-center text-xs font-bold text-gray-700 uppercase w-40">공고기관</th>
                                    <th className="px-2 py-3 text-center text-xs font-bold text-gray-700 uppercase w-20">현황</th>
                                    <th className="px-2 py-3 text-center text-xs font-bold text-gray-700 uppercase w-20">
                                    ...
                                    <th className="px-3 py-3 text-center text-xs font-bold text-gray-700 uppercase w-20">
                                    ...
                                    <th className="px-3 py-3 text-center text-xs font-bold text-gray-700 uppercase w-24">과제별 지원금<br /><span className="text-[10px] lowercase">(단위: 억)</span></th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-200">
                                {projects.map((project, index) => (
                                    <tr key={project.id} className="hover:bg-blue-50/50 transition">
                                        <td className="px-3 py-3 text-center text-sm font-medium text-gray-900">{totalCount - (currentPage - 1) * 20 - index}</td>
                                        <td className="px-3 py-3 text-center">
                                            <a href={project.URL} target="_blank" className="text-sm text-gray-900 hover:text-[#4169E1] hover:underline">{project.title}</a>
                                        </td>
                                        <td className="px-3 py-3 text-center">
                                            <button onClick={() => toggleAgency(project.organization)} className="text-[13px] text-[#4169E1] hover:underline font-medium">{project.organization}</button>
                                        </td>
                                        <td className="px-3 py-3 text-center">
                                        ...
                                        <td className="px-3 py-3 text-center text-[12px] text-gray-600">{project.start_date}</td>
                                        <td className="px-3 py-3 text-center">
                                            <div className="text-[12px] text-gray-600">{project.end_date}</div>
                                            ...
                                            {editingBudgetId === project.id ? (
                                                <div className="flex flex-col items-center gap-1">
                                                    ...
                                                    <div className="flex gap-1">
                                                        ...
                                                        <button onClick={cancelBudgetEdit} className="px-1 bg-gray-300 text-[9px] rounded">취소</button>
                                                    </div>
                                                </div>
                                            ) : (
                                                <div className="text-[12px] text-gray-700 font-medium hover:bg-blue-50 px-2 py-1 rounded transition">{project.budget || '-'}</div>
                                            )}
                                        </td>
                                    </tr>
                                ))}
                ...
                <div className="mt-8 flex justify-center items-center space-x-1">
                    <button onClick={() => {
                        ...
                        className="p-2 text-gray-600 hover:text-[#4169E1] disabled:opacity-30 disabled:cursor-not-allowed"
                    >
                        «
                    </button>
                    ...
                    {Array.from({ length: Math.min(10, totalPages) }, (_, i) => {
                        ...
                        return pageNum <= totalPages ? (
                            <button key={pageNum} onClick={() => setCurrentPage(pageNum)} className={`w-10 h-10 flex items-center justify-center text-sm font-medium rounded-full transition ${currentPage === pageNum ? 'bg-[#4169E1] text-white' : 'text-gray-600 hover:text-[#4169E1]'}`}>{pageNum}</button>
                        ) : null;
                    })}
                ...
                <div className="mt-6 text-center text-sm text-gray-600">검색된 정보: <span className="font-bold text-[#4169E1]">{totalCount} 건</span></div>
            </main>
        </div>
    );
};
