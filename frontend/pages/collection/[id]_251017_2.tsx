...
export default function CollectionDetailPage() {
  ...
  const iconMapper = (type?: string) => {
    switch (type) {
      ...
      default:
        return <Folder className="h-6 w-6 text-white" />;
    }
  };
  ...
  if (!collection) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-white">
        <p className="text-zinc-500">데이터를 불러올 수 없습니다.</p>
      </div>
    );
  }
    ...
    <>
      ...
      <div className="min-h-screen bg-white p-8">
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          <Link
            href="/collection"
            className="inline-flex items-center gap-2 text-sm text-zinc-600 hover:text-blue-600 transition-colors"
          >
            <ArrowLeft size={16} />
            컬렉션 목록으로
          </Link>
            ...
            <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-8 py-6">
              <div className="flex items-center gap-4">
                ...
                <div className="text-right">
                  ...
                  <div className="text-blue-100 text-sm">건</div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-zinc-100 overflow-hidden">
            <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-8 py-6">
              <h2 className="text-xl font-semibold text-white">컬렉션 리스트</h2>
            </div>
              ...
              <div className="overflow-hidden rounded-xl border-2 border-zinc-200">
                <table className="w-full">
                  <thead className="bg-zinc-50">
                    <tr>
                      <th className="px-4 py-3 text-left text-sm font-semibold text-zinc-700 w-20">번호</th>
                      <th className="px-4 py-3 text-left text-sm font-semibold text-zinc-700">제목</th>
                      <th className="px-4 py-3 text-left text-sm font-semibold text-zinc-700">출원인</th>
                      <th className="px-4 py-3 text-left text-sm font-semibold text-zinc-700">출원번호</th>
                      <th className="px-4 py-3 text-left text-sm font-semibold text-zinc-700">출원일</th>
                    </tr>
                  </thead>
                  <tbody>
                    {dataItems.length > 0 ? (
                      dataItems.map((d, idx) => (
                        <tr key={d.id} className="border-t border-zinc-200 hover:bg-blue-50 transition-colors">
                          ...
                          <td className="px-4 py-3 text-sm text-zinc-700">{d.applicant_name ?? "-"}</td>
                          <td className="px-4 py-3 text-sm font-mono text-zinc-700">{d.application_number ?? "-"}</td>
                ...
                {totalPages > 1 && (
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 border-t border-zinc-200 bg-zinc-50">
                    <div className="flex items-center gap-2">
                      <button onClick={() => setPage(1)} disabled={page === 1} className="px-3 py-2 border-2 border-zinc-200 rounded-lg disabled:opacity-50 hover:bg-white transition-all">«</button>
                      <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="px-3 py-2 border-2 border-zinc-200 rounded-lg disabled:opacity-50 hover:bg-white transition-all">이전</button>
                      <span className="px-4 py-2 text-sm font-medium">
                        <span className="text-blue-600 font-bold">{page}</span> / {totalPages}
                      </span>
                      <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="px-3 py-2 border-2 border-zinc-200 rounded-lg disabled:opacity-50 hover:bg-white transition-all">다음</button>
                      <button onClick={() => setPage(totalPages)} disabled={page === totalPages} className="px-3 py-2 border-2 border-zinc-200 rounded-lg disabled:opacity-50 hover:bg-white transition-all">»</button>
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      <select
                        ...
                        className="px-3 py-2 border-2 border-zinc-200 rounded-lg text-sm"
                      >
                        {[10, 20, 50, 100].map((n) => (
