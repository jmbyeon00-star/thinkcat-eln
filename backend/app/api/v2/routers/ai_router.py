...
def get_models(
    ...
    # import os
    # FRONTEND_URL = os.getenv("FRONTEND_URL", "kkk")
...
# @router.get("/{model_id}")
# def get_model(model_id: int, session: Session = Depends(get_sync_session)):
#     return session.query(ModelInfo).filter(ModelInfo.id == model_id).first().__dict__

# @router.get("/project/{project_id}")
# def get_models_by_project(project_id: int, session: Session = Depends(get_sync_session)):
#     return session.query(ModelInfo).filter(ModelInfo.project_id == project_id).all()

@router.get("/{model_id}")
def get_model_detail(model_id: int, request: Request, session: Session = Depends(get_sync_session)):
    try:
        user_id = get_current_user_from_request(request)
    except Exception as e:
        print("get_current_user_from_request raised:", repr(e))
...
async def retrain_project_data(
    ...
    shuffle: str = Form(...),
    use_file: str = Form(...),
    selected_groups: Optional[str] = Form(None), # JSON 문자열로 받음
    file: Optional[UploadFile] = File(None),     # 실제 파일 객체
...
async def get_history(file_id: int, db: Session = Depends(get_sync_session)):
...
async def auto_recommend(
    ...
    try:
        ...
        return await ai_service.run_auto_recommend(
            session, user_id, collection_id, body
        )
...
async def check_recommendation_ready(collection_code: str, session: Session = Depends(get_sync_session)):
    """
    추천 학습 가능 상태 점검:
    1. 이미 학습된 모델 존재 여부
    2. 이후 추가된 데이터 존재 여부
...
def get_recommendation_result(model_id: int, request: Request, session: Session = Depends(get_sync_session)):
    """
