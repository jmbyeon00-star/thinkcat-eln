...
async def delete_collections_and_data(session: AsyncSession, user_id: int, project_id: int, items: list):
    try:
        ...
        target_codes = [item.collection_code for item in items]

        # 1) 데이터 및 컬렉션 삭제
        await session.execute(delete(ProjectData).where(ProjectData.project_id == project_id, ProjectData.collection_code.in_(target_codes)))
        await session.execute(delete(CollectionInfo).where(CollectionInfo.id.in_(target_ids), CollectionInfo.user_id == user_id))
        await session.flush()
...
def get_project_collections(session: Session, user_id: int, project_id: int, page: int, limit: int, q: str) -> Dict[str, Any]:
    ...
    model_info = session.query(ModelInfo).filter(ModelInfo.user_id == user_id, ModelInfo.data_id == project_id).all()
...
def insert_project_data(session: Session, user_id: int, project_id: int, group_code: str, body: dict):
    ...
    try:
        ...
        # 데이터 추출
        correct_data = body.get("items", [])      # Positive 데이터 (used=1)
        counter_data = body.get("n_items", [])  # Negative 데이터 (used=0)

        if not correct_data and not counter_data:
            return {"message": "No data provided"}
        
        # 1) 컬렉션 맵(ID 확보) 생성 로직
        collection_map = {}
        all_col_names = set([
            doc.get("label") or doc.get("collection_name") 
            for doc in correct_data 
            if doc.get("label") or doc.get("collection_name")
        ])

        if counter_data:
            all_col_names.add("COUNTER")
        ...
        def add_project_data(items, used_flag):
            now = datetime.now()
            for doc in items:
                app_num = doc.get("application_number", None)
                ...
                if existing:
                    ...
                    existing.collection_name = col_name
                    ...
                    session.add(ProjectData(
                        ...
                        collection_id=c_id, collection_code=c_code, collection_name=col_name,
        ...
        # 3) 실제 데이터 삽입 실행
        add_project_data(correct_data, used_flag=1)
        add_project_data(counter_data, used_flag=0)
        session.flush() # DB에 데이터 반영

        # 추가) COUNTER 데이터 취급 관련
        total_processed_docs = session.query(ProjectData).filter(
            ProjectData.project_id == project_id,
            ProjectData.used.in_([0, 1])
        ).count()
        ...
        for c in all_collections:
            ...
            real_count = session.query(ProjectData).filter_by(project_id=project_id, collection_id=c.id).count() # counter data도 함께 취급
            c.collection_data_num = real_count
            # c.collection_data_ratio = (real_count / total_docs) if total_docs > 0 else 0
            c.collection_data_ratio = (real_count / total_processed_docs) if total_processed_docs > 0 else 0 # counter data도 함께 취급
            c.updated_datetime = datetime.now()
        ...
        labeled_num = session.query(ProjectData).filter_by(project_id=project_id, used=1).count() # counter data도 함께 취급
