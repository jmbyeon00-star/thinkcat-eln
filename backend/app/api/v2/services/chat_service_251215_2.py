        ...
        async with httpx.AsyncClient(timeout=None) as client:
            async with client.stream(
                ...
                async for line in resp.aiter_lines():
                    ...
                    token = line.replace("data:", "").rstrip()
                    if token == "[DONE]" or token.startswith("["):
                        continue
                    
                    sentence_buffer = append_token_ko(sentence_buffer, token)
                    ...
                    should_flush = (
                        token in [".", "!", "?"]
                        or len(sentence_buffer) >= 20
                        or (now - last_flush_time) >= 0.35
                    )
                    
                    # if should_flush(token):
                    if should_flush:
                        assistant_text += sentence_buffer
