...
def get_project(project_id: int, session: Session = Depends(get_session), source_type: str = Query(None)):
    # if source_type:
...
def get_project_and_models(project_id: int, request: Request, session: Session = Depends(get_session)):
    '''
