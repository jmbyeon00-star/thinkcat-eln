...
class PatentInfo(Base):
    ...
    # 등록번호
    reg_number: Mapped[str] = mapped_column("RGSTNO", String(20), index=True, nullable=False)
...
class PatentLiti(Base):
    __tablename__ = "PATENT_LITI_TB"

    uid: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    applicant_code: Mapped[int | None] = mapped_column('applicant_code', index=True)
    name: Mapped[str | None] = mapped_column(String(255))
    transfer_cnt: Mapped[int | None] = mapped_column(Integer)
    apply_cnt: Mapped[int | None] = mapped_column(Integer)
    defendant: Mapped[int | None] = mapped_column(Integer)
    plaintiff: Mapped[int | None] = mapped_column(Integer)