...
from app.utils.security import verify_password, generate_code
...
def signup(data: Signup, db: Session = Depends(get_db)):
    ...
    FRONTEND_ORIGIN = os.getenv("ALLOWED_ORIGINS", "http://localhost:3000")
    verify_link = f"{FRONTEND_ORIGIN}/auth/verify?email={new_user.email}&code={code}"
...
def signin(data: Signin, db: Session = Depends(get_db)):
    print("[/auth/signup] received raw:", data)
    # return {"debug": data.dict()}
...
def resend_verification(email: EmailStr, db: Session = Depends(get_db)):
    ...
    verify_link = f"{FRONTEND_ORIGIN}/verify?email={user.email}&code={code}"
