...
def build_es_query_from_intent(intent: dict, options: dict, query_vector=None):
    try:
        filters = []
        should = []
...
async def search_with_mcp(payload: dict):
    user_query = payload.get('body', {}).get('query')
    options = payload.get('body', {}).get('options', {})
    # results = await search_with_ollama(user_query)

    # 1. 검색 의도 분석
    intent = await analyze_search_intent(user_query)
    print("\n\n\n>>> intent:", intent)
    ...
    # 3. ES DSL 생성
    query_dsl = build_es_query_from_intent(intent, options, query_vector)
    print("\n\n\n>>> query_dsl:", query_dsl)
