...
def infer_classification(config: dict):
    ...
    try:
        query = """
            SELECT model_code, data_scope, source_type, task_type, collection_num
            FROM MODEL_INFO_TB
    ...
    try:
        ...
        if results["has_ground_truth"]:
            query = """
                INSERT INTO FILE_RESULT_TB(
                    user_id, model_id, model_code, file_code, input_text, predicted_label, predicted_labels, confidence, probability, ground_truth
                ) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            """
            for item in results["results"]:
                cursor.execute(query, (
                    user_id, model_id, model_code, file_code, 
                    item["input_text"], item["predicted_label"], str(item["predicted_labels"]), item["confidence"], str(item["probability"]), item["ground_truth"]
                ))
                
            query = """
                UPDATE FILE_INFO_TB SET accuracy=%s, correct_count=%s, total_count=%s, error_count=%s
                WHERE id=%s AND user_id=%s
            """
            cursor.execute(query, (
                results["evaluation"]["accuracy"], 
                results["evaluation"]["correct_count"], 
                results["evaluation"]["total_count"],
                results["evaluation"]["error_count"],
                file_id, user_id
            ))
        else:
            query = """
            ...
            for item in results["results"]:
                cursor.execute(query, (
                    user_id, model_id, model_code, file_code, 
                    item["input_text"], item["predicted_label"], item["predicted_labels"], item["confidence"], item["probability"]
                ))
...
def infer_recommendation(config: dict):
    ...
    try:
        query = """
            SELECT model_code, source_type, task_type, collection_num
            FROM MODEL_INFO_TB
            WHERE id=%s AND user_id=%s
        """
        cursor.execute(query, (model_id, user_id))
