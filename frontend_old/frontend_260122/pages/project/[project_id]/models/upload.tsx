  ...
  useEffect(() => {
    ...
    async function loadProject() {
      try {
        ...
        const currentSubPath = currentPath.split('/').pop()?.toLowerCase(); // 예: 'upload'
        const requiredSubPath = data.source_type.toLowerCase(); // 예: 'search'

        if (currentSubPath && currentSubPath !== requiredSubPath) {

          // 필수 타입이 'search'라면, '/project/[project_id]/data/search'로 이동
          const newPath = `/project/${project_id}/models/${requiredSubPath}`;

          // router.replace를 사용하여 히스토리를 오염시키지 않고 경로 교체
          router.replace(newPath);

          // 리디렉션을 시도했으므로, 이 컴포넌트의 렌더링을 중단하거나 추가 로직 실행을 막을 수 있습니다.
          // setProject는 유지하여 리디렉션된 후에도 데이터가 유지되도록 합니다.
          return;
        }
        // <
