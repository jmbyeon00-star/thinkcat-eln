...
export default function RecommendationPanel({
    ...
    //     // 3. [추가 로직] 실시간으로 학습 완료(progress === -1)를 감지했을 때
    //     // storeProgress는 SSE를 통해 실시간으로 변하는 값입니다.
    //     if (storeProgress === -1 && modelInfo?.id) {
    //         // 파일 시스템 저장 시간을 위해 2초 대기 후 결과 호출
    //         const timer = setTimeout(() => {
    //             fetchRecommendationResult(modelInfo.id);
    //             // 학습이 끝났으므로 상태 초기화 (필요시)
    ...
    useEffect(() => {
        ...
        checkModel();
    }, [collection.collection_code]); // 🚀 token이나 hasTrainedModel을 빼고 '코드'가 바뀔 때만!

    console.log(">>> params:", params)

    // 2. SSE를 통한 실시간 완료 감지 (별도 분리)
    useEffect(() => {
        // storeProgress가 -1이 되는 순간은 SSE 컴포넌트가 값을 바꿨을 때임
        if (storeProgress === -1 && modelInfo?.id) {
            console.log("🏁 Training completed detected via SSE. Fetching results...");

            const timer = setTimeout(() => {
                fetchRecommendationResult(modelInfo.id);
                // 🚀 중요: 상태를 AVAILABLE로 바꿔서 다시 버튼이 보이게 함
                setState({ isBusy: false, status: 'AVAILABLE', progress: 0 });
            }, 2000);

            return () => clearTimeout(timer);
        }
    }, [storeProgress, modelInfo?.id]); // 🚀 storeProgress의 변화에만 집중
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-zinc-100 overflow-hidden">
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-6 border border-blue-100">
                        ...
                        {isBusy && (storeStatus === "RUNNING" || storeStatus === "INFERRING") && modelInfo ? (
                            /* ✅ 학습 중일 때 SSE 진행률 표시 */
                            <div className="flex items-center justify-between">
                                {/* {storeStatus === "RUNNING" && '신규 데이터 학습'} */}
                                {/* {storeStatus === "INFERRING" && '추론'} */}
                                <span className="text-sm font-medium text-blue-600 animate-pulse">
                                    {storeStatus === "RUNNING" ? '신규 데이터 학습 중...' : '추천 결과 생성 중...'}
                                </span>
                                <span className="text-xs text-zinc-400">ID: {modelInfo.id}</span>
                                <ModelProgressSSE
