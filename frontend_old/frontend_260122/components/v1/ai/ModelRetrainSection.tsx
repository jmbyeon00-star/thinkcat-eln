...
import { ModelDetail, Props, RetrainRow } from "@/types/ai";

// export default function ModelDetailPage() {
const ModelRetrainSection: React.FC<Props> = ({
    model,
    isBusy,
    storeStatus,
    token,
    ...
    const [showRetrain, setShowRetrain] = useState(true);
    ...
    const lineData = useMemo(() => {
        ...
        model?.history?.segments.forEach((seg, segIndex) => {
            ...
            for (let i = 0; i < seg.train_acc.length; i++) {
                rows.push({
                    key: `seg-${segIndex}-${i}`,
                    accuracy: seg.valid_acc[i],
                    loss: seg.valid_loss[i],
                    epoch: epochCounter++,
    ...
    const handleReTrain = () => {
        if (!model) {
            console.error("Model data is not loaded yet.");
            // onStateChange({ isBusy: false, status: "AVAILABLE" });
            return;
        }
        ...
        fetch(`${API_BASE}/api/ai/retrain/classification/${model.data_scope}/${model.project_id || model.collection_id}`, {
            method: "POST",
            headers: {
        ...
        <div className="rounded-xl border border-zinc-200 bg-white shadow-sm overflow-hidden">
            ...
            >
                <div className="flex items-center gap-2" >
                    ...
                    <h2 className="text-lg font-semibold text-white">추가학습 (Fine-Tuning)</h2>
                </div>
            </div>

            {showRetrain && (
                <div className="p-6">
                    {(model?.updated_count ?? 0) >= 100 ? (
                        <>
                            {model && isBusy && storeStatus === "RUNNING" || storeStatus === "INFERRING" && model?.id ? (
                                ...
                                <div>
                                    ...
                                    <ModelProgressSSE
                                        targetId={model.id}
                                        initialProgress={model.progress}
                                ...
                                <>
                                        ...
                                        <div className="flex items-start gap-4">
                                            ...
                                            <div className="flex-none">
                                                ...
                                                <p className="text-sm text-zinc-600 leading-relaxed">
                                                    이 모델에 사용된 프로젝트에 새로운 데이터가 {model.updated_count}개 확인 되었습니다.
                                                    <br />
                                    ...
                                    <button
                                        className="w-full flex items-center justify-center mt-4 gap-2 px-6 py-3 border border-blue-600 text-blue-600 rounded-xl hover:bg-blue-600 hover:text-white transition-all font-medium"
                        ...
                        <div className="bg-gradient-to-br from-yellow-50 to-orange-50 rounded-2xl shadow-sm border border-yellow-200 p-8">
                            <div className="flex items-start gap-4">
                                ...
                                <div className="flex-none">
                                    <h3 className="text-lg font-semibold text-zinc-900 mb-2">
                                        해당 모델은 추가 학습이 불가능합니다.
                                    </h3>
                                    <p className="text-sm text-zinc-600 leading-relaxed">
                                        해당 모델에 사용된 프로젝트에 새로운 데이터를 추가하세요
                                    </p>
                                </div>
                            </div>
                        </div>
...
export default ModelRetrainSection;